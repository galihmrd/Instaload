    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        Btn.setOnClickListener() {
            if (Box.text.toString() != "") {
                ...
                if (Box.text.toString().startsWith("https://")) { // checks if the text is a valid instagram link
                    // Post shortcode is a part of the Post URL

                    CoroutineScope(Dispatchers.IO).launch {
                        try {
                            linkDownloader?.call(Box.text.toString())
                            runOnUiThread {
                                Toast.makeText(
                                    this@MainActivity,
                                    "Download Finished",
